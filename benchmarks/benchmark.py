...
class BaseBenchmark(ABC):
    ...
    def save_results_to_csv(self, results: List[Tuple[Any, ...]], columns: List[str]):
        # Filter out None results (samples skipped due to API failures)
        valid_results = [r for r in results if r is not None]
        skipped_count = len(results) - len(valid_results)

        if skipped_count > 0:
            logger.warning(f"Skipped {skipped_count} samples due to API errors. Valid samples: {len(valid_results)}/{len(results)}")
