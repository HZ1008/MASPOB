...
class GraphUtils:
    def __init__(self, root_path: str, auto_create_round_dirs: bool = True):
        """Utility helpers for graph / workflow files.

        Args:
            root_path: Dataset-specific root, e.g. ``workspace/DROP``.
            auto_create_round_dirs: Whether to automatically create ``round_k``
                directories on demand. Set True for the Optimizer; set False for
                WorkflowEvaluator to avoid creating empty directories during eval.
        """
        self.root_path = root_path
        self.auto_create_round_dirs = auto_create_round_dirs
    ...
    def create_round_directory(self, graph_path: str, round_number: int) -> str:
        ...
        if self.auto_create_round_dirs:
